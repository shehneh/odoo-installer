...
# On Liara, disk is mounted at /usr/src/app/data
# Use environment variable or check for Liara platform
import platform
IS_LINUX = platform.system() == 'Linux'
LIARA_DATA_PATH = Path('/usr/src/app/data')

if IS_LINUX and LIARA_DATA_PATH.parent.exists():
    # We're on Liara - use the mounted disk
    DATA_DIR = LIARA_DATA_PATH
    try:
...
# Private downloadable artifacts
# On Liara: use the mounted disk (writable)
# On local: use private_downloads folder
if IS_LINUX and LIARA_DATA_PATH.parent.exists():
    DOWNLOAD_FILES_DIR = DATA_DIR / "installers"
else:
...
def seed_admin():
    """Create default admin user if no users exist - for initial setup on Liara"""
    secret = request.json.get('secret') if request.json else None
    if secret != 'odoomaster2025':
        return jsonify({'error': 'Unauthorized'}), 401
    
    users = load_json(USERS_FILE)
    if users:
        return jsonify({'message': 'Users already exist', 'count': len(users)}), 200
    
    # Create default admin
    admin_email = 'shehneh@gmail.com'
    users[admin_email] = {
        'id': '960efd8db6ea156e',
        'name': 'محمد شحنه',
        'email': admin_email,
    ...
    try:
        ...
        return jsonify({'success': True, 'message': 'Admin user created'}), 201
...
# ============================================
# Admin File Upload (for installers)
# ============================================

UPLOAD_SECRET = os.environ.get('UPLOAD_SECRET', 'odoomaster2025upload')

@app.route('/api/admin/upload-installer', methods=['POST'])
def admin_upload_installer():
    """Upload installer file to server (admin only, secret key required)"""
    ...
    try:
        DOWNLOAD_FILES_DIR.mkdir(parents=True, exist_ok=True)
        file_path = DOWNLOAD_FILES_DIR / filename
...
@app.route('/api/admin/list-installers', methods=['GET'])
def admin_list_installers():
    ...
    try:
        files = []
        if DOWNLOAD_FILES_DIR.exists():
            for f in DOWNLOAD_FILES_DIR.iterdir():
                if f.is_file():
                    files.append({
                        'name': f.name,
                        'size': f.stat().st_size
                    })
        return jsonify({'files': files, 'dir': str(DOWNLOAD_FILES_DIR)})
    except Exception as e:
        return jsonify({'error': str(e)}), 500
